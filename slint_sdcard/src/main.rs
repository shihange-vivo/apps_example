...
// Seven rows fit between the list origin (y=144) and the footer (y=436).
const MAX_VISIBLE_ENTRIES: usize = 7;
...
const TEXT_PAGE_COLUMNS: usize = 54;
const TEXT_PAGE_LINES: usize = 16;
...
const DEMO_ROOT: &str = "/data/slint-demo";
const DEMO_MARKER: &str = "/data/slint-demo/README.txt";
...
/// Populates a private demo directory once, leaving any other SD card contents untouched.
/// The marker is written last so a failed partial run can safely be retried after reboot.
fn populate_demo_tree() -> IoResult<String> {
    std::fs::create_dir_all(DEMO_ROOT)?;
    populate_large_text_samples()?;
    populate_png_sample()?;

    if std::fs::metadata(DEMO_MARKER).is_ok() {
        println!("[SDCARD-DEMO] reusing existing {DEMO_ROOT}");
        return Ok(DEMO_ROOT.to_string());
    }

    let now = uptime_millis();
    let mut rng = DemoRng::new((now as u32) ^ ((now >> 32) as u32) ^ 0xb10e_05c6);

    for category in ["docs", "photos", "logs"] {
        let directory = format!("{DEMO_ROOT}/{category}-{:04x}", rng.next() & 0xffff);
        std::fs::create_dir_all(&directory)?;
        println!("[SDCARD-DEMO] created directory {directory}");

        for index in 1..=3 {
            let path = format!("{directory}/item-{index}-{:04x}.txt", rng.next() & 0xffff);
            let contents = format!(
                "BlueOS SD card demo file\ncategory={category}\nitem={index}\nrandom={:08x}\n",
                rng.next()
            );
            std::fs::write(&path, contents.as_bytes())?;
            println!("[SDCARD-DEMO] created file {path}");
        }
    }

    for index in 1..=6 {
        let path = format!("{DEMO_ROOT}/note-{index}-{:04x}.txt", rng.next() & 0xffff);
        let contents = format!(
            "Random text file #{index}\nvalue={:08x}\nGenerated by slint_sdcard_example.\n",
            rng.next()
        );
        std::fs::write(&path, contents.as_bytes())?;
        println!("[SDCARD-DEMO] created file {path}");
    ...
    let mut binary = [0u8; 512];
    for index in 1..=3 {
        rng.fill(&mut binary);
        let length = 128 + (rng.next() as usize % 385);
        let path = format!("{DEMO_ROOT}/sample-{index}-{:04x}.bin", rng.next() & 0xffff);
        let mut file = std::fs::File::create(&path)?;
        file.write_all(&binary[..length])?;
        println!("[SDCARD-DEMO] created file {path} ({length} bytes)");
    }

    std::fs::write(
        DEMO_MARKER,
        b"BlueOS Slint SD card browser demo\nThis directory is safe to remove.\n",
    )?;
    println!("[SDCARD-DEMO] generated test tree in {DEMO_ROOT}");
    Ok(DEMO_ROOT.to_string())
}

fn populate_large_text_samples() -> IoResult<()> {
    for (name, line_count) in [
        ("long-guide.txt", 48usize),
        ("long-report.txt", 84usize),
        ("limit-demo.txt", 140usize),
    ] {
        let path = format!("{DEMO_ROOT}/{name}");
        let expected_minimum_size = line_count as u64 * 64;
        if std::fs::metadata(&path)
            .map(|metadata| metadata.len() >= expected_minimum_size)
            .unwrap_or(false)
        {
            continue;
        }
        ...
        for line in 1..=line_count {
            let token = (line as u32).wrapping_mul(0x045d_9f3b);
            writeln!(
                file,
                "{line:03} | BlueOS SD pagination sample. This line is intentionally long; token={token:08x}."
            )?;
        }
        file.flush()?;
        let size = std::fs::metadata(&path)?.len();
        println!("[SDCARD-DEMO] created large text file {path} ({size} bytes)");
    }
    Ok(())
}
...
fn populate_png_sample() -> IoResult<()> {
    const WIDTH: u32 = 480;
    const HEIGHT: u32 = 480;
    let path = format!("{DEMO_ROOT}/gradient-480.png");
    if std::fs::metadata(&path)
    ...
    {
        let mut stream = writer.stream_writer_with_size(2048).map_err(png_error)?;
        let mut row = [0u8; WIDTH as usize * 3];
        for y in 0..HEIGHT {
            for x in 0..WIDTH {
                let offset = x as usize * 3;
                row[offset] = (x * 255 / (WIDTH - 1)) as u8;
                row[offset + 1] = (y * 255 / (HEIGHT - 1)) as u8;
                row[offset + 2] = if (x / 48 + y / 48) % 2 == 0 { 220 } else { 48 };
            }
    ...
    let size = std::fs::metadata(&path)?.len();
    println!("[SDCARD-DEMO] created PNG sample {path} ({WIDTH}x{HEIGHT}, {size} bytes)");
    Ok(())
}
...
fn to_slint_entry(entry: &FileEntryInfo) -> FileEntry {
...
fn push_text_character(
    ...
    if *column != 0 && *column + width > TEXT_PAGE_COLUMNS {
        finish_text_line(page, pages, line, column);
    }
    page.push(character);
    *column += width;
}

fn paginate_text(contents: &str) -> Vec<String> {
    ...
    for character in contents.chars() {
        match character {
            '\r' => {}
            '\n' => finish_text_line(&mut page, &mut pages, &mut line, &mut column),
            '\t' => {
                let spaces = 4 - column % 4;
                for _ in 0..spaces {
                    push_text_character(' ', &mut page, &mut pages, &mut line, &mut column);
                }
            }
            character if character.is_control() => {
                push_text_character('\u{fffd}', &mut page, &mut pages, &mut line, &mut column)
            }
            character => {
                push_text_character(character, &mut page, &mut pages, &mut line, &mut column)
            }
        }
    }
    ...
    if pages.is_empty() {
        pages.push("(empty file)".to_string());
    }
    pages
}
    ...
    fn open_text_file(&mut self, ui: &MainWindow, path: &str, name: &str, size: u64) {
        match read_text_pages(path, size) {
            Ok((pages, truncated)) => {
                ...
                ui.set_text_viewer_title(name.into());
    ...
    fn open_png_file(&mut self, ui: &MainWindow, path: &str, name: &str) {
        match inspect_png(path) {
            Ok(header) => {
                ...
                ui.set_image_viewer_title(name.into());
                ui.set_image_viewer_path(path.into());
                ui.set_image_viewer_status(
                    format!(
                        "{}x{}  ->  {}x{}  |  RGB565",
                        header.width, header.height, display_width, display_height
                    )
                    .into(),
                );
    ...
    load_timer.start(
        ...
        move || {
            if let Some(ui) = initial_ui.upgrade() {
                let demo_path = populate_demo_tree();
                let mut browser = browser.borrow_mut();
                match demo_path {
                    Ok(path) => browser.current_path = path,
                    Err(error) => {
                        println!("[SDCARD-DEMO] failed to create test content: {error}")
                    }
                }
                browser.refresh(&ui);
            }
